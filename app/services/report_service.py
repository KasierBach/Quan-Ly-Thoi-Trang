...
class ReportService(BaseService):
    ...
    def get_dashboard_stats():
        """Retrieve core dashboard statistics."""
    ...
    def get_revenue_report(start_date, end_date, group_by='day'):
        """Generate detailed revenue report data."""
        conn = BaseService.get_connection()
        try:
            with conn.cursor() as cursor:
                if not start_date:
                    ...
                    start_date = min_row[0].strftime('%Y-%m-%d') if min_row and min_row[0] else (datetime.now() - timedelta(days=365)).strftime('%Y-%m-%d')
                ...
                for row in daily_rows:
                    ...
                    if group_by == 'week':
                        ...
                        display_date = datetime.strptime(f'{year}-W{week}-1', "%G-W%V-%u").date()
                    elif group_by == 'month':
                    ...
                    if date_key not in aggregated_data:
                        aggregated_data[date_key] = {'OrderDate': display_date, 'DailyRevenue': 0, 'OrderCount': 0}
                ...
                # Mock object class for dot notation compatibility in templates
                class ReportRow:
                    def __init__(self, d):
                        self.OrderDate = d['OrderDate']
                        self.DailyRevenue = d['DailyRevenue']
                        self.OrderCount = d['OrderCount']

                daily_revenue = [ReportRow(aggregated_data[k]) for k in sorted(aggregated_data.keys())]
                ...
                return {
                    ...
                    'daily_revenue': daily_revenue, 'category_revenue': category_revenue,
                    'best_selling': best_selling, 'new_customers_range': cursor.fetchone()[0],
                    'group_by': group_by
                }
        finally:
            conn.close()
            ...
            with conn.cursor() as cursor:
                ...
                if report_type == 'daily_revenue':
                    ...
                    for r in cursor.fetchall():
                        writer.writerow([r.OrderDate.strftime('%d/%m/%Y') if r.OrderDate else 'N/A', r.OrderCount, float(r.DailyRevenue or 0)])
                        
                elif report_type == 'best_selling':
                    cursor.execute('''
                        SELECT p.ProductName, SUM(od.Quantity) as TotalSold, SUM(od.Quantity * od.Price) as Revenue
                        FROM OrderDetails od JOIN ProductVariants pv ON od.VariantID = pv.VariantID
                        JOIN Products p ON pv.ProductID = p.ProductID JOIN Orders o ON od.OrderID = o.OrderID
                        WHERE o.OrderDate BETWEEN %s AND %s GROUP BY p.ProductID, p.ProductName
                        ORDER BY TotalSold DESC LIMIT 20
                    ''', (start_date, end_date))
                    writer.writerow(['Tên sản phẩm', 'Số lượng bán', 'Doanh thu (VNĐ)'])
                    for r in cursor.fetchall(): writer.writerow([r[0], r[1], float(r[2] or 0)])
                    ...
                    cursor.execute('''
                        ...
                        FROM OrderDetails od JOIN ProductVariants pv ON od.VariantID = pv.VariantID
                        JOIN Products p ON pv.ProductID = p.ProductID JOIN Categories c ON p.CategoryID = c.CategoryID
                        JOIN Orders o ON od.OrderID = o.OrderID WHERE o.OrderDate BETWEEN %s AND %s
                    ...
                    cursor.execute('''
                        ...
                        WHERE o.OrderDate BETWEEN %s AND %s GROUP BY c.CustomerID, c.FullName, c.Email
                        ORDER BY TotalSpent DESC LIMIT 20
                    ''', (start_date, end_date))
                    writer.writerow(['Tên khách hàng', 'Email', 'Số đơn hàng', 'Tổng chi tiêu (VNĐ)'])
                    for r in cursor.fetchall(): writer.writerow([r[0], r[1], r[2], float(r[3] or 0)])
                    ...
                    cursor.execute('''
                        ...
                        JOIN Colors c ON pv.ColorID = c.ColorID JOIN Sizes s ON pv.SizeID = s.SizeID
                        WHERE pv.Quantity < 20 ORDER BY pv.Quantity ASC LIMIT 50
                    ''')
                    writer.writerow(['Sản phẩm', 'Màu', 'Size', 'Số lượng còn'])
                    for r in cursor.fetchall(): writer.writerow([r[0], r[1], r[2], r[3]])
                        
                elif report_type == 'order_details':
                    cursor.execute('''
                        SELECT o.OrderID, c.FullName, o.OrderDate, o.TotalAmount, o.Status, o.PaymentMethod
                        FROM Orders o JOIN Customers c ON o.CustomerID = c.CustomerID
                        WHERE o.OrderDate BETWEEN %s AND %s ORDER BY o.OrderDate DESC LIMIT 100
                    ''', (start_date, end_date))
                    writer.writerow(['Mã đơn', 'Khách hàng', 'Ngày đặt', 'Tổng tiền (VNĐ)', 'Trạng thái', 'Thanh toán'])
                    for r in cursor.fetchall():
                        writer.writerow([r[0], r[1], r[2].strftime('%d/%m/%Y %H:%M') if r[2] else 'N/A', float(r[3] or 0), r[4], r[5] or 'N/A'])
